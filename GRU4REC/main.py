...
parser.add_argument('--data_folder', default='trivagoRecSysChallengeData2019/processed_data', type=str)
parser.add_argument('--train_data', default='train_tr.csv', type=str)
parser.add_argument('--valid_data', default='validation.csv', type=str)
...
def main():
    ...
    if not args.is_eval:
        make_checkpoint_dir()
    train_dataset, validation_dataset = lib.MiniBatchedDataset.get_all_dataset(
        os.path.join(args.data_folder, 'item_metadata.csv'),
    ...
    input_size = len(train_dataset.reference_classes)
    hidden_size = args.hidden_size
    num_layers = args.num_layers
    output_size = len(train_dataset.reference_classes)
    ...
    if not args.is_eval:
        ...
        if args.load_model is not None:
            ...
            loss, recall, mrr = evaluation.eval(test_dataset)
